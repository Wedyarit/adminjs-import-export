...
import { ActionProps, ApiClient, useNotice, useLocalStorage } from 'adminjs';
...
const ExportComponent: FC<ActionProps> = ({ resource }) => {
  const [localFilters] = useLocalStorage('localFilters', {});
  ...
  const exportData = async (type: ExporterType) => {
    ...
    try {
      ...
      const blob = new Blob([Buffer.from(exportedData, 'base64')], {
        type: mimeTypes[type],
      });
